...
import com.kabasoft.iws.client.SPAMain.{DashboardLoc, Loc}
import com.kabasoft.iws.gui.macros.Bootstrap._
import com.kabasoft.iws.gui.macros.{BootstrapStyles, GlobalStyles,Icon, Refresh}
...
object MainMenu {
  ...
  case class Props(router: RouterCtl[Loc], currentLoc: Loc, position:Int, name:String, proxy: ModelProxy[Option[Int]],loc:Loc,instance:IWS)
  ...
  private def menuItems (position:Int,loc:Loc) = {
    if (position ==1) {
    Seq(
      MenuItem(0, _ => "Dashboard", Icon.dashboard, DashboardLoc),
  ...
  def apply(ctl: RouterCtl[Loc], currentLoc: Loc, position:Int, name:String, proxy: ModelProxy[Option[Int]],loc:Loc, instance:IWS): ReactElement =
