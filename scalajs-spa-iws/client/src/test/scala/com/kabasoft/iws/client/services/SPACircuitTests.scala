...
import com.kabasoft.iws.shared.Model.{Store => MStore}
...
object SPACircuitTests extends TestSuite {
  def tests = TestSuite {
   'IWSHandler - {
    // val ss = await()
       val store = Ready(Store(Map(4 ->Ready(Data(
         Seq(
       ...
       val motd=Ready("XXXX")
       def build = new IWSHandler(new RootModelRW(store.asInstanceOf[Pot[Store[IWS,IWS]]]))
     ...
     assert(result == ModelUpdate(Ready(Store(Map(4 ->Ready(Data(newTodos)))))))
   }
