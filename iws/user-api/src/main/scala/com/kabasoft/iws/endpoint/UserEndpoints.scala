...
class UserEndpoints[F[_]: Sync, A, Auth: JWTMacAlgo] extends Http4sDsl[F] {
  ...
  private def list(userService: UserService[F]): AuthEndpoint[F, Auth] = {
    case GET -> Root :? PageSizeMatcher(pageSize) :? OffsetMatcher(offset) asAuthed _ =>
      for {
        retrieved <- userService.list(pageSize.getOrElse(100000), offset.getOrElse(0))
        resp <- Ok(retrieved.asJson)
      } yield resp
  }

  private def searchByName(userService: UserService[F]): AuthEndpoint[F, Auth] = {
  ...
  private def deleteUser(userService: UserService[F]): AuthEndpoint[F, Auth] = {
  ...
  def endpoints(
    ...
    val authEndpoints: AuthService[F, Auth] =
      com.kabasoft.iws.auth.Auth.adminOnly {
        ...
        update(userService)
          .orElse(list(userService))
          .orElse(searchByName(userService))
          .orElse(deleteUser(userService))
      } //)

    val unauthEndpoints =
      login(userService, cryptService, auth.authenticator) <+>
        signup(userService, cryptService)
