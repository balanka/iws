...
import com.kabasoft.iws.domain.{Bank, CostCenter}
...
import com.kabasoft.iws.repository.doobie.{BankService, CostCenterService, User}
...
class BankEndpoints[F[_]: Sync, Auth: JWTMacAlgo] extends Http4sDsl[F] {
  ...
  implicit val ccDecoder: EntityDecoder[F, CostCenter] = jsonOf[F, CostCenter]
  ...
  private def ccList(service: CostCenterService[F]): AuthEndpoint[F, Auth] = {
    case req @ POST -> Root asAuthed _ =>
      for {
        cc <- req.request.as[CostCenter]
        created <- service.create(cc)
        resp <- Created(created.asJson)
      } yield resp
    case DELETE -> Root / id asAuthed user =>
      service.delete(id, user.company) *> Ok()
    ...
    case req @ PATCH -> Root asAuthed user =>
      for {
        cc <- { println("req.request.as[CostCenter]" + req.request.as[CostCenter]); req.request.as[CostCenter] }
        updated <- service.update(cc, user.company)
        response <- Ok(updated.asJson)
      } yield response
  ...
  private def getCC(service: CostCenterService[F]): AuthEndpoint[F, Auth] = {
    case GET -> Root / id asAuthed user =>
      service.getBy(id, user.company).flatMap {
        case Some(found) => Ok(found.asJson)
        case None => NotFound("")
  ...
  def endpoints(
    ...
    val authEndpoints: AuthService[F, Auth] = {
      ...
      Auth.allRolesHandler(ccList(ccService).orElse(getCC(ccService))) {
        Auth.adminOnly(ccList(ccService).orElse(getCC(ccService)))
      }
    }
...
object BankEndpoints {
  def endpoints[F[_]: Sync, Auth: JWTMacAlgo](
    ...
    ccService: CostCenterService[F],
    auth: SecuredRequestHandler[F, Long, User, AugmentedJWT[Auth, Long]]
  ): HttpRoutes[F] = new BankEndpoints[F, Auth].endpoints(service, ccService, auth)
}
